...
### 2E — Engine Quality + Observability ✅ COMPLETE (2026-05-31, PRs #10–12)
**9 of 11 items shipped.** Two intentionally deferred: extraction types (text/table/reasoning) — needs table extraction first; output format normalization — separate follow-on plan. Full review pane and partial batch retry landed as deferred-but-required items.
...
**Shipped (PRs #11–12) — required before Phase 3, now complete:**
- *Full document review pane* ✅ — `SchemaReviewPane.jsx` maps over `schema.schema_fields`; every field visible alongside PDF, pre-populated where extracted, empty where not. Four field states (auto-extracted, low confidence, not extracted, obscured). PDF highlight overlay + text layer search (`useFieldHighlight`). Region capture for evidence (`useRegionCapture`). New `POST .../extractions` endpoint creates attempt=3 rows for fields with no prior extraction. OCR bumped to 300 DPI. Spec: `docs/superpowers/specs/2026-05-30-full-document-review-pane-design.md`.
- *Partial batch retry* ✅ — when `batch_errors > 0` but `< len(batches)`, failed fields are collected and retried once as `call_type="batch_retry_partial"`. Transient API errors resolve silently. If retry also fails, document routes to the review pane where empty fields are visible and fillable.
...
### 2F — Commercial Readiness + Data Connectors (pre-Phase 3) — PLANS 1–3 COMPLETE ✅
...
**Plan 1 ✅ COMPLETE (2026-05-31):** Commercial readiness — prompt caching, model routing, usage metering foundation.  
**Plan 2 ✅ COMPLETE (2026-05-31):** Connector backend — ConnectorBase contract, IRS TEOS connector, connector service + registry, connector endpoints, connector_runs table, document source provenance.  
**Plan 3 ✅ COMPLETE (2026-05-31):** Sources UI + digital viewer + AI suggestion — Sources page (search→pick→pull), DigitalDocumentRenderer + rendererRegistry, DocumentViewer pluggable left pane, source badge in DocumentList, suggest_source agent tool (7th), SuggestSourceCard action card, workspace document_render_mode preference.
...
- FBI IC3 format
- Example Lender Administration OIG
