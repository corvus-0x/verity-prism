...
- Good: `expect(screen.getByText('Bright Future Ministries Inc')).toBeInTheDocument()`
  ...
  server.use(
    http.post('/workspaces/ws-1/search', () =>
      HttpResponse.json({
        ...
        results: [{
          document_id: 'doc-1',
          filename: '2023-09-22_DEED_Keller_13-Trinity-Zero.pdf',
          detected_doc_type: 'DEED',
          matched_fields: { consideration_amount: '0', grantee_name: 'Keller Family' },
        }]
      })
    )
  )
...
3. Create a workspace: "Bright Future Ministries Inc", vertical: fraud
