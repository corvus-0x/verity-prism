...
Think about what you did manually during your investigation: you downloaded a PDF from the county auditor, read through it, and noted that Sarah Mitchell paid $300,000 for a property worth $37,490. This platform does that reading and noting automatically, for every document, every time.
...
This platform was built from a real fraud investigation (Bright Future Ministries Inc, EIN 12-3456789, Madison County OH). That investigation is **Vertical 1** — the first specific use case. Insurance form automation is **Vertical 2**. Other industries follow the same pattern.
...
name            — "Bright Future Ministries Inc" or "March 2026 Auto Claims Batch"
...
jurisdiction    — where this applies (fraud: "Madison County, OH"; insurance: a region)
...
[
  {
    ...
    "required": true,
    "example": "Bright Future Real Estate LLC"
  },
  {
    "name": "grantee_name",
    "type": "name",
  ...
  },
  {
    ...
    "required": false,
    "example": "47 Sycamore St, Cedar Grove OH 45351"
  },
  {
    "name": "parcel_number",
    "type": "id",
...
### 6.7 entities
The people, organizations, and properties that appear in a workspace. Often auto-populated from document extractions — if Claude extracts "Sarah Mitchell" as a grantor on 5 deeds, the system can suggest creating a Person entity for her.

```
id           — unique identifier
workspace_id — which workspace
type         — "person", "organization", "property", or "financial_account"
name         — "Sarah Mitchell", "Bright Future Ministries Inc", "47 Sycamore St"
...
**Why separate `amount_paid` from `appraised_value`?** This is how you catch overpayments and underpayments automatically. When Sarah Mitchell paid $300,000 for a property appraised at $37,490, that's `amount_paid = 300000` and `appraised_value = 37490` — an 800% overpayment. With structured fields, the system can flag this automatically.
...
**Pre-loaded signal types from the Bright Future investigation:**
...
title           — "47 Sycamore St — 700% overpayment to Winner Kyle J"
...
id                  — unique identifier
workspace_id        — which workspace
question            — "Does Sarah Mitchell have related businesses in Ohio?"
...
**Real examples from the Bright Future investigation:**
```
2024-01-03_DEED_BrightFutureRealEstate_25-W-Main-St.pdf
2022-09-15_990_BrightFutureInHisName_TaxYear2021.pdf
2019-08-22_SOS-FILING_BrightFutureRealEstateLLC_Formation.pdf
2020-07-17_UCC_MitchellJay_SBA-EIDL-Lien.pdf
2023-03-28_DEED_BrightFutureInHisName_135-Elm-St-Overpayment.pdf
```
...
- *"show me documents mentioning Sarah Mitchell"*
- *"list all deeds where the paid amount was more than double the appraised value"*
- *"what documents were filed between 2019 and 2022 in Madison County"*
...
Step 7:  Claude generates the standardized filename
         → Uses the extracted data to produce: 2024-01-03_DEED_BrightFutureRealEstate_...
