...
- Farm Credit Mid-America, PCA as secured party is an agricultural lender — note connection to EIDL liens if both cover the same debtor.
...
[
  ...
  {"name": "entity_number", "type": "id_number", "required": false,
   "description": "SOS entity number — permanent ID for this entity (e.g. 4371988)"},
  {"name": "filing_date", "type": "date", "required": true,
   "description": "Date the filing was made with the SOS"},
...
- The filing attorney/firm appearing across multiple entity filings (e.g. Hanes Law Group on all Do Good filings) establishes a professional network connection.
...
- The same contractor appearing across multiple properties (e.g. Baumer Construction on all Do Good properties) establishes a network connection.
...
**Extraction notes:**
- `memorial_donation_destinations` is high-value — it reveals which organizations the deceased was connected to. A DBA name appearing here (e.g. "Do Good Ministries" instead of "Do Good In His Name Inc") may flag an unregistered DBA.
- `survivors_listed` should capture ALL names, relationships, and locations — this maps the family network.
- Cross-reference survivor locations against known addresses in the case (Maria Stein, Osgood, Celina, etc.).
