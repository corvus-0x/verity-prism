...
| `pages/workspace/DocumentViewer.jsx` | `.../documents/:id` | Split-pane PDF viewer (65%) + extracted fields panel (35%). react-pdf renders in-browser, no plugin needed. Status-aware fields panel surfaces extraction_error on failure. Field clicks now jump to the saved evidence region or first cross-page text-layer match and draw the overlay in both normal and review modes. | ✅ |
...
#### `hooks/useFieldHighlight.js` ✅
Engine. Text-layer match hook for field values. Exports the pure `findFieldTextMatches()` matcher used by both the live overlay and cross-page page discovery. Sliding-window token matching handles multi-word values in recorder-style prose where labels are absent.

#### `components/documents/pdfFieldLinking.js` ✅
Engine. PDF field-linking helpers. Builds page search order, caches pdf.js text content per page, reuses saved evidence regions when present, and finds the first matching page across the whole document when evidence does not exist yet.
...
| 2026-06-19 | Field-level linking completed. `DocumentViewer` now replays saved evidence regions or searches cached pdf.js text across all pages to jump to the first match; overlay works in both standard viewer and review mode. `ExtractionTable` forwards field focus metadata. Added `pdfFieldLinking.js` helper module and `test_field_linking.jsx` (4 targeted frontend tests). |
