...
def test_get_document_file(client, auth_headers, workspace_id):
    # Patch the pipeline: this test covers file serving, not extraction.
    # Unpatched, the pipeline fails in CI (no tesseract, no API key) and
    # its failure handler deletes the stored file, turning the GET into 404.
