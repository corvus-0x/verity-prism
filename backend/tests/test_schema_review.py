...
def ws_schema_doc(db, auth_headers, client):
    ...
    ws_resp = client.post(
        "/workspaces/", json={"name": "Review WS", "vertical": "general"}, headers=auth_headers
    )
    ...
    schema = DocumentSchema(
        ...
        vertical="general",
        schema_fields=[
            {"name": "grantor_name", "type": "name", "description": "Grantor", "group": "Parties"}
        ],
        version=1,
        is_active=True,
...
def test_create_extraction_404_for_unknown_doc(client, auth_headers, ws_schema_doc):
    ...
    resp = client.post(
        ...
        json={"field_name": "x", "field_value": "y", "field_type": "text", "schema_id": schema.id},
        headers=auth_headers,
    )
    assert resp.status_code == 404


def test_create_extraction_rejects_field_with_prior_extraction(
    client, auth_headers, ws_schema_doc, db
):
    ws_id, schema, doc = ws_schema_doc
    db.add(
        DocumentExtraction(
...
def test_create_extraction_rejects_schemaless_document(client, auth_headers, ws_schema_doc, db):
    ws_id, schema, _ = ws_schema_doc
    user = db.query(User).filter(User.email == "analyst@example.com").first()
    doc2 = Document(
        id=str(uuid.uuid4()),
        workspace_id=ws_id,
        filename="noschema.pdf",
        original_filename="noschema.pdf",
        file_path="/tmp/noschema.pdf",
        file_type="pdf",
        sha256_hash="noschema123",
        uploaded_by=user.id,
        extraction_status="needs_review",
        schema_id=None,
    )
    ...
    db.commit()
    resp = client.post(
        f"/workspaces/{ws_id}/documents/{doc2.id}/extractions",
        json={
            "field_name": "grantor_name",
            "field_value": "y",
            "field_type": "name",
...
def test_create_extraction_flips_doc_to_complete_when_all_fields_resolved(
    client, auth_headers, ws_schema_doc, db
):
