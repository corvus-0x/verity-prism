...
def test_update_deleted_finding_returns_404(client, auth_headers, workspace_id, db):
    ...
    finding = client.post(
        f"/workspaces/{workspace_id}/findings",
        json={"title": "Test", "severity": "high", "signal_type_id": signal["id"]},
        headers=auth_headers,
    ).json()
    row = db.query(Finding).filter(Finding.id == finding["id"]).first()
    row.is_deleted = True
    db.commit()
    response = client.patch(
        f"/workspaces/{workspace_id}/findings/{finding['id']}",
