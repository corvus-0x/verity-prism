...
def test_evaluate_flags_fields_below_threshold():
    extractions = [
        ...
        {"field_name": "parcel_id", "field_value": "35-001-234", "confidence": 0.30},
    ]
    result = evaluate(extractions, threshold=0.75)
...
def test_extraction_sends_full_text_beyond_4000_chars(db, deed_schema):
    ...
    # Content is now a list of cacheable blocks (static prefix + document text),
    # so flatten to text before checking the marker reached Claude.
    sent = captured_prompts[0]
    text = sent if isinstance(sent, str) else "".join(b.get("text", "") for b in sent)
    assert "GRANTOR_EVIDENCE_AT_5000" in text, (
