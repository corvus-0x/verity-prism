...
def document(db, workspace, user):
    ...
    db.execute(
        text(
            "UPDATE documents SET search_vector = to_tsvector('english', :content) WHERE id = :id"
        ),
...
def test_search_documents_workspace_isolation(db, workspace, document, user):
    other_ws = Workspace(
        id=str(uuid.uuid4()), name="Other WS", vertical="fraud", created_by=user.id
    )
    db.add(other_ws)
    db.commit()
...
def test_get_entity_exact_match(db, workspace, entity):
    result = get_entity(workspace_id=workspace.id, db=db, name="Bright Future Ministries Inc")
    assert result["entity"]["name"] == "Bright Future Ministries Inc"
    assert result["entity"]["data"]["ein"] == "12-3456789"


def test_get_entity_partial_match(db, workspace, entity):
    result = get_entity(workspace_id=workspace.id, db=db, name="Bright Future")
    assert result["entity"]["name"] == "Bright Future Ministries Inc"
...
def test_get_entity_workspace_isolation(db, workspace, entity, user):
    ...
    db.add(other_ws)
    db.commit()
    result = get_entity(workspace_id=other_ws.id, db=db, name="Bright Future")
    assert result["entity"] is None
...
def test_get_entity_case_insensitive(db, workspace, user):
    ...
    db.add(e)
    db.commit()
    result = get_entity(workspace_id=workspace.id, db=db, name="bright future ministries")
    assert result["entity"]["name"] == "Bright Future Ministries Inc"
...
def test_get_entity_excludes_soft_deleted(db, workspace, user):
    ...
    deleted = Entity(
        id=str(uuid.uuid4()),
        workspace_id=workspace.id,
        name="Bright Future Deleted Org",
        type="organization",
        status="active",
        is_deleted=True,
        created_by=user.id,
    ...
    db.add(deleted)
    db.commit()
    result = get_entity(workspace_id=workspace.id, db=db, name="Bright Future Deleted")
    assert result["entity"] is None
...
def test_query_extractions_eq(db, workspace, extraction):
    result = query_extractions(
        workspace_id=workspace.id, db=db, field_name="grantor", operator="eq", value="John Smith"
    )
    assert result["count"] == 1
...
def test_query_extractions_contains(db, workspace, extraction):
    result = query_extractions(
        workspace_id=workspace.id, db=db, field_name="grantor", operator="contains", value="Smith"
    )
    assert result["count"] == 1
    assert result["extractions"][0]["filename"] == "test_deed.pdf"
    assert result["extractions"][0]["doc_type"] == "DEED"
...
def test_query_extractions_no_match(db, workspace, extraction):
    result = query_extractions(
        workspace_id=workspace.id, db=db, field_name="grantor", operator="eq", value="Nobody"
    )
    assert result["count"] == 0
...
def test_query_extractions_excludes_soft_deleted_document(db, workspace, user):
    ...
    result = query_extractions(
        workspace_id=workspace.id, db=db, field_name="grantor", operator="eq", value="Ghost Grantor"
    )
    assert result["count"] == 0
...
def test_get_transactions_zero_amount_serializes_as_string(db, workspace, user):
    ...
    assert (
        float(txn["amount_paid"]) == 0.0
    )  # stored as 0.00 by Postgres; Decimal("0") is falsy — must use is not None
