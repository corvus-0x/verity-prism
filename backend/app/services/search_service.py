...
def semantic_search(query: str, workspace_id: str, db: Session, limit: int = 10) -> list[dict]:
    """
    Find documents by semantic similarity to query text.
...
def run_search(workspace_id: str, query_plan: dict, db: Session, mode: str = "hybrid") -> list[dict]:
    ...
    semantic_results = semantic_search(
        query_plan.get("fts_query") or query_plan.get("doc_type_filter") or "",
        workspace_id,
        db,
    )
