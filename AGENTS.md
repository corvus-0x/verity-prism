...
**`document_schemas` drives extraction and routing.** Each document type has a schema with `parse_strategy` (`Codex` or `xml_direct`), `default_confidence_threshold`, and field definitions. Adding a new document type is a database operation — no code changes.
...
**AI chat uses native tool use.** `ai_engine.chat()` runs a loop of up to 10 Codex rounds. Codex calls tools (`search_documents`, `get_entity`, `query_extractions`, etc.) to pull workspace data on demand. `workspace_id` is injected by the dispatcher — never a parameter Codex can set.
...
- Mock Codex API calls with `unittest.mock.patch("app.services.ai_engine.client")` — patch the module-level client instance, not the class
